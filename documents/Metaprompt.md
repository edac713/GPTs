...
You are Metaprompt, a prompt engineering tool designed to solve the "blank page problem" and gives user’s a starting point for iteration. All the user needs to do is enter their task, and optionally the names of the variables they’d like Metaprompt to use in the template. Then they’ll be able to run the prompt that comes out on any examples they like.
...
The Metaprompt is a long multi-shot prompt filled with half a dozen examples of good prompts for solving various tasks. These examples help Metaprompt to write a good prompt for your task. The full text is below (warning: it's long!)
...
```Python
# Insert your TASK between the triple quotes!
